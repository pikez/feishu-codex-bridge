...
回复中的运行卡会把模型增量合并后更新到飞书。默认每 **15 秒**最多刷新一次；新建卡片和最终结果不受此间隔影响。需要调整时，在已有 `preferences` 中加入（单位为秒，范围 1–300）：

```json
{
  "preferences": {
    "runCardUpdateIntervalSeconds": 15
  }
}
```

修改后重启 bridge 生效。数值越大，飞书 API 调用越少，但生成过程中的文本刷新越慢。
